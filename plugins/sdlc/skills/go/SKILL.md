...
argument-hint: "<one-sentence request> [--autopilot|--no-autopilot] [--merge|--no-merge] [--no-pr] [--slug <slug>]"
...
## Inputs
- `$ARGUMENTS`: the request in one sentence; `--autopilot` / `--merge` = skip the plan pause / merge when checks are green — on a solo repository both are already the default (`init` sets `roles.autopilot` and `roles.auto_merge` to true), so they are only needed where the config says false; `--no-autopilot` / `--no-merge` = add the pause back for this run; `--no-pr` = stop after the verified commit; `--slug` = reuse an existing chain (when it names an approved intent and no request text follows, the intent's title and Problem section are the request — this is how `/sdlc:run` calls this skill).
- `.sdlc/config.json`: `roles.solo`, `roles.autopilot` (default for `--autopilot`), `roles.auto_merge` (default for `--merge`), `commands.*`, `policies`, `paths.*`, `auto_commit`. Solo default is fully automatic; a pause is something the owner adds, not something the loop assumes.
...
0. **Preflight** (no questions). `bash "${CLAUDE_PLUGIN_ROOT}/scripts/doctor.sh" --json`: if `.sdlc/config.json` is missing, stop with one line — "run `/sdlc:init` first (it auto-detects GitHub, solo and CI auth); it has to be typed by you" — because init is reserved for explicit user invocation and cannot be called from a skill. Read `roles.solo` and `roles.autopilot`. If the repo is not git, stop: the loop needs a commit trail. Derive `slug` from the request (kebab-case, ≤ 5 words) unless `--slug`. Record `started` (ISO time) for the `go.run` event.
   **Autopilot eligibility**: autopilot applies when (a) `--autopilot` was typed, or the user invoked `/sdlc:go` directly and `roles.autopilot` is true — never when this skill was auto-invoked by the model from a phrase, and never with `--no-autopilot`; and (b) `roles.solo` is true. Doctor's `auto_mode` readiness (tuned CLAUDE.md, policy skills, hooks, a test suite Claude can run) is advisory here: if it is not `ok`, say in one line which guardrail is missing and that `roles.autopilot: false` restores the plan pause — then continue; the reviewer, the checks and the hooks still gate the merge.
...
7. **Pull request** (unless `--no-pr`). Set `status: implemented` in `plan/<slug>.md` (once the PR merges, this is how `/sdlc:run` and `/sdlc:status` know the item is done), commit, `git push -u origin sdlc/<slug>`, `gh pr create` with body: intent/spec/plan links, the Proof output, the review tally line (`SDLC_REVIEW_TALLY …`). Then run the babysit loop from `/sdlc:review --pr <n>` once: address `sdlc review` workflow comments and failing checks until green.
   - auto-merge on (`roles.auto_merge` true or `--merge` typed, and no `--no-merge`) and `roles.solo`: `gh pr merge --auto --squash --delete-branch <n>`. GitHub accepts auto-merge only when `allow_auto_merge` is on **and** the base branch is protected; when it refuses (typically a private Free-plan repository without rulesets — "clean status" / "not allowed"), do not merge blind: `gh pr checks <n> --watch` until every check has completed green, then `gh pr merge --squash --delete-branch <n>`. If a check fails, fix and re-push (babysit loop) — never merge with a failing or still-running check. Otherwise (team repository, or the owner switched auto-merge off) **stop** here: "PR #n is green and waiting for your merge".
8. **Report** briefly: slug, the four artifact paths and commits, verify output tail, review tally, PR URL, cost/turn note, and what (if anything) needs a human now. Suggest `/sdlc:release staging` when a deploy command exists.
...
| Plan — only when autopilot is off (team repository, `roles.autopilot: false`, or `--no-autopilot`) | The playbook's build gate, kept where a person wants it |
| Verification failing after 3 rounds | Continuing would mean weakening the check |
| Merge — only when auto-merge is off (team, `roles.auto_merge: false`, or `--no-merge`); any production deploy (`RELEASE_APPROVAL`) always | Separation of duties |
...
- Never merge when auto-merge is off (`roles.auto_merge: false` without `--merge`, or `--no-merge`); never set `RELEASE_APPROVAL`; never disable a hook to get past a block — report it instead. Solo repositories without a server-side ruleset (private, Free plan) are protected by the hooks and this skill's discipline alone — the merge conditions (checks green, reviewer Important 0) do not relax there.
